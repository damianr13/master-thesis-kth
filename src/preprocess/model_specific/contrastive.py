...
class ContrastivePreprocessor(BasePreprocessor[ContrastivePreprocessConfig]):
    ...
    def _extract_relevant_columns(self, df_for_location: Dict[str, DataFrame]) -> Dict[str, DataFrame]:
        ...
        pretrain_set_df = result.pop(ContrastivePreprocessor.PRETRAIN_SET_FILENAME)

        result = super()._extract_relevant_columns(result)
        result[ContrastivePreprocessor.PRETRAIN_SET_FILENAME] = pretrain_set_df
        return result
    ...
    def __extract_standalone_clusters(self, full_set: DataFrame, full_matches: DataFrame, id_column: str):
        ...
        result = result.groupby(by=id_column).aggregate({
            id_column: ContrastivePreprocessor.__get_unique_set_aggregator
        })
    ...
    def __assign_clusters(self, target: DataFrame,
        ...
        result = target.merge(cluster_assignation_left.rename(columns={
            ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME: f'left_{ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME}'
        }),
        ...
        result = result.merge(cluster_assignation_right.rename(columns={
            ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME: f'right_{ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME}'
        }),
    ...
    def _preprocess_one(self, df: DataFrame) -> DataFrame:
        ...
        textual_columns.remove(ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME)
    ...
    def _preprocess_all(self, df_for_location: Dict[str, DataFrame]) -> Dict[str, DataFrame]:
        ...
        cluster_assignation_left = clusters_df.explode(self.config.left_id_column).rename(
            columns={self.config.left_id_column: ContrastivePreprocessor.ITEM_ID_COLUMN_NAME})
        cluster_assignation_left = cluster_assignation_left[[ContrastivePreprocessor.ITEM_ID_COLUMN_NAME,
                                                             ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME]]

        cluster_assignation_right = clusters_df.explode(self.config.right_id_column).rename(
            columns={self.config.right_id_column: ContrastivePreprocessor.ITEM_ID_COLUMN_NAME})
        cluster_assignation_right = cluster_assignation_right[[ContrastivePreprocessor.ITEM_ID_COLUMN_NAME,
                                                               ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME]]

        result = {k: self.__assign_clusters(v, cluster_assignation_left, cluster_assignation_right)
                  for k, v in df_for_location.items()}

        return self.__separate_sources_pretrain(result)
    ...
    def __separate_sources_pretrain(self, df_for_location: Dict[str, DataFrame]) -> Dict[str, DataFrame]:
        ...
        result = df_for_location.copy()
        result[ContrastivePreprocessor.PRETRAIN_SET_FILENAME] = pretrain_set
        return result

    def __separate_sources_for_one(self, df: DataFrame) -> DataFrame:
        df = df.sample(frac=self.config.pretrain_sample)

        columns_for_instance = ['text', ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME]
        ...
        left_instances_df = left_instances_df[
            left_instances_df[ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME].notnull()].drop_duplicates()
        right_instances_df = right_instances_df[
            right_instances_df[ContrastivePreprocessor.CLUSTER_ID_COLUMN_NAME].notnull()].drop_duplicates()
